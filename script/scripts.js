...
const activitiesName = document.getElementById("activitiesName");
activitiesName.onchange = activitySelectorChanged;
const categoryTag = document.getElementById("categories");
categoryTag.onchange = categorySelected;
const resetBtn = document.getElementById("resetBtn");
resetBtn.onclick = resetBtnClicked;

for(let i=0; i<categories.length; i++) {
  const theOption = new Option(categories[i], categories[i]);
  categoryTag.appendChild(theOption);
}

function categorySelected() {
  const value = categoryTag.options[categoryTag.selectedIndex].textContent;
  for(let i=0; i<activities.length; i++) {
    if(value == activities[i].category){
      const selectedActivity = new Option(activities[i].name, activities[i].id);
      activitiesName.appendChild(selectedActivity);
      activitiesName.size = activitiesName.length;
  }
 }
}

function activitySelectorChanged() {
  const activitiesValue = activitiesName.options[activitiesName.selectedIndex].value;
  for(let i=0; i<activities.length; i++) {
    if(activitiesValue == activities[i].id) {
      activityName.innerHTML = activities[i].name;
      activityDescription.innerHTML = activities[i].description;
      activityPrice.innerHTML = activities[i].price;
      document.getElementById("activityDetails").style.display = 'block';
    }
    if(activitiesValue == activities[i].id && activities[i].price > 0.0) {
      activityName.innerHTML = activities[i].name;
      activityDescription.innerHTML = activities[i].description;
      activityPrice.innerHTML = activities[i].price;
      document.getElementById("activityDetails").style.display = 'block';
...
function resetBtnClicked() {
  categoryTag.options[categoryTag.selectedIndex].textContent = 'Select One';
  activitiesName.length = 0;
  activitiesName.size = 0;
  document.getElementById("activityName").innerHTML = '';
  document.getElementById("activityDescription").innerHTML = '';
  document.getElementById("activityPrice").innerHTML = '';
  displayActivity.options[displayActivity.selectedIndex].textContent = displayActivity.options[0].textContent;
